...
def reconfigureConstraints_helper(graph, constraints):

	nodeArr = []

	if("children" in graph):
		for child in graph["children"]:
			returnValue = reconfigureConstraints_helper(child, constraints)
			if(returnValue != None):
	...
	globalVar.runningTime -= timeElapsed
...
def reconfigureConstraints_func(graph, constraints):

	reconfigureConstraints_helper(graph, constraints)
